...
class MyApi
{
    ...
    private function doSaveTokens($credentials, $new_tokens_json)
    {
        ...
        if(!empty($tokens_json)) { // update
            $stmt = $this->db->prepare("UPDATE tokens SET tokens=? WHERE username=? AND client_id=? AND client_secret=?");
    ...
    public function postContacts($credentials, $json = "")
    {
        $api_response = false;
        ...
        if(empty($json)) {
            ...
            foreach ($rows as $key=> $row) {
             ...
             //   $rows[$key]['AccountNumber']=$accountNumber;

                $arr = [
                    'ContactID' => $row['xero_id'],
                  ...
                  /*  'FirstName'=>$row['person_first_name'],
                    ...
                    'IsCustomer'=>true,
                    'DefaultCurrency'=>'AUD',
                    'ContactPersons'=>$arr_contact_person,
                    'ContactNumber' =>0 */
                ];
...
//var_dump($data_arr);exit;
        ...
        $defaults = array(
            CURLOPT_URL => $credentials['endpoint_uri'] . '/v1/xero/post-contacts?accessToken=' . $tokens['access_token'],
            CURLOPT_POST => true,
            CURLOPT_POSTFIELDS => $params,
            CURLOPT_RETURNTRANSFER => true
        );        
    ...
    public function postItems($credentials, $json = "")
    {
        $api_response = false;
        ...
        $defaults = array(
            CURLOPT_URL => $credentials['endpoint_uri'] . '/v1/xero/post-items?accessToken=' . $tokens['access_token'],
            CURLOPT_POST => true,
            CURLOPT_POSTFIELDS => $params,
            CURLOPT_RETURNTRANSFER => true
        );        
    ...
    public function postInvoices($credentials, $json = "{}")
    {
        $api_response = false;
