...
class ctl_settlement extends adminPage
{
	...
	{	
		...
		if($this->user['role']==6){
		    	...
		    	$businessList = $mdl_user->getList('id,businessName,displayName,name,password',array('role'=>3,'user_belong_to_agent'=>$_SESSION['admin_user_id'],'settlement_type'=>$type));
			
		}else{
				$businessList = $mdl_user->getList('id,businessName,displayName,name,password',array('role'=>3,'settlement_type'=>$type));
