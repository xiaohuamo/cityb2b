...
class ctl_index extends adminPage
{
	...
	function ctl_index ()
	{
		...
		{
			case 'today'	: $where[] = "t0.publishedDate='".date('Y-m-d', time())."'"; break;
			case 'week'		:
				$tmpArr		= getIntervalDays('week');
				$where[]	= "t0.publishedDate>='".date('Y-m-d', $tmpArr[0])."' and t0.publishedDate<='".date('Y-m-d', $tmpArr[1])."'";
				break;
			case 'month'	:
	...
	public function add_action () #act_name = 添加信息#
	{
		...
		if (is_post())
		{
			if ($data = self::_filter(post('data')))
			{
				...
				if (trim($data['publishedDate']) == '') $data['publishedDate'] = date('Y-m-d', time());
				$tags = $data['tags'];
					...
					if ( $_FILES[$upload_file]['size'] > 0 ) {
						...
						if ( $filename ) {
							...
							if ( $upload_file == 'imageUrl' ) {
								...
								if ( $class['other']['pic1width'] > 0 && $class['other']['pic1height'] > 0 && $bs[4] - $bs[2] > 0 && $bs[5] - $bs[3] > 0 ) {
								...
								$bs = explode(',', $bounds[$boundsIndex]);
								if ( $class['other']['pic2width'] > 0 && $class['other']['pic2height'] > 0 && $bs[4] - $bs[2] > 0 && $bs[5] - $bs[3] > 0 ) {
									$this->file->resize( UPDATE_DIR.$filename, UPDATE_DIR.$filename, $bs[0], $bs[1], true, true );
									$this->file->cutByPosBound( UPDATE_DIR.$filename, UPDATE_DIR.$filename, array( 'x1' => $bs[2], 'y1' => $bs[3], 'x2' => $bs[4], 'y2' => $bs[5] ) );
						...
						foreach ( $pics['name'] as $key => $name ) {
							...
							if ( $pics['size'][$key] > 0 ) {
								...
								if ( in_array( $ext, $exts ) ) {
									...
									$bs = explode(',', $bounds[$boundsIndex]);
									if ( $class['other']['infopic2width'] > 0 && $class['other']['infopic2height'] > 0 && $bs[4] - $bs[2] > 0 && $bs[5] - $bs[3] > 0 ) {
		...
		if (is_post())
		{
			...
			if ($data = self::_filterForEdit(post('data')))
			{
				...
				if ($info->update($this->array_splice($data, 'tags', 1), $id))
				{
					...
					//self::_html($id);
					$this->sheader($this->returnUrl, $upload_error > 0 ? $this->lang->edit_info_success_but_upload_pic_has_error : null);
				}
				else {
					if ( $new_files ) {
						$this->file->deletefile( $new_files );
