...
class mdl_info extends mdl_base
{
	...
	{
		return $this->db->toArray($this->db->select(null, $this->tableName, array('classId' => $class_id, 'isApproved' => 1), $orderBy, $start.', '.$cnt));
	}
	...
	public function getPreviousOfSameClass($id){
		$row = $this->get($id);
		$classid = $row['classId'];

		return $this->getByWhere(array("id > ". $id,'classId = ' .$classid));		
	}

	public function getNextOfSameClass($id){
		$row = $this->get($id);
		$classid = $row['classId'];

		//return $this->getByWhere(array("id < ". $id,'classId = ' .$classid));
		$sql = "select * from ".$this->tableName." where id < ".$id." and classId = ".$classid." order by id desc limit 1";
