   ...
   public static  function getAvaliableDateOfAllSalesChannelOfThisFactory($businessId) {
	   ...
	   public static  function getSqlofAllOrdersDataOfCurrentBusiness($businessId,$tableName,$factroy_id,$totalandeverychannelPrint) {
		...
		if(!$totalandeverychannelPrint) {
		  ...
		  if($businessId) {
			$sql =" select o.logistic_delivery_date,c.business_id,cate.category_sort_id,r.menu_order_id,cate.category_cn_name,cate.category_en_name,r.source_menu_id as main_code_id,r.menu_id,r.menu_en_name,r.unit,if(length(r.menu_en_name)>0,r.menu_en_name,c.bonus_title) as bonus_title,c.guige_des,sum(c.customer_buying_quantity) as  total_quantity from cc_wj_customer_coupon c left join $tableName o on c.order_id =o.orderId left join cc_restaurant_menu r on c.restaurant_menu_id =r.id left join  cc_restaurant_category  cate on r.restaurant_category_id =cate.id";
			$sql .= " where (c.business_id =$businessId or c.business_id in (select customer_id from cc_factory2c_list where factroy_id =$businessId)  or c.business_id in (select customer_id from cc_factory_2blist where factroy_id =$businessId) )  and o.coupon_status='c01' ";
				...
				$sql =" select o.logistic_delivery_date,cate.category_sort_id,r.menu_order_id,cate.category_cn_name,cate.category_en_name,r.source_menu_id as main_code_id,r.menu_id,r.menu_en_name,r.unit,if(length(r.menu_en_name)>0,r.menu_en_name,c.bonus_title) as bonus_title,c.guige_des,sum(c.customer_buying_quantity) as  total_quantity from cc_wj_customer_coupon c left join $tableName o on c.order_id =o.orderId left join cc_restaurant_menu r on c.restaurant_menu_id =r.id left join  cc_restaurant_category  cate on r.restaurant_category_id =cate.id";
			 ...
			 $sql =" select c.business_id,u.displayName,o.logistic_delivery_date,cate.category_sort_id,r.menu_order_id,cate.category_cn_name,cate.category_en_name,r.source_menu_id as main_code_id,r.menu_id,r.menu_en_name,r.unit,if(length(r.menu_en_name)>0,r.menu_en_name,c.bonus_title) as bonus_title,c.guige_des,sum(c.customer_buying_quantity) as  total_quantity from cc_wj_customer_coupon c left join $tableName o on c.order_id =o.orderId left join cc_restaurant_menu r on c.restaurant_menu_id =r.id left join cc_user u on c.business_id =u.id left join  cc_restaurant_category  cate on r.restaurant_category_id =cate.id";
			 $sql .= " where (c.business_id =$factroy_id or c.business_id in (select customer_id from cc_factory2c_list where factroy_id =$factroy_id) or c.business_id in (select customer_id from cc_factory_2blist where factroy_id =$factroy_id))  and o.coupon_status='c01' ";
			//var_dump($sql);exit;
		}
