	   ...
	   public static  function getSqlofAllOrdersDataOfCurrentBusiness($businessId,$tableName,$factroy_id,$totalandeverychannelPrint) {
		...
		if($allDefinedItem =='allDefinedItem') {
         ...
         //   var_dump($whereProductRange);exit;
              ...
              if($businessId) {
                  $sql =" SELECT r.source_menu_id,c.restaurant_menu_id ,r.menu_id,c.guige1_id,c.guige_des,r.menu_cn_name,r.menu_en_name,if(length(r.unit_en)>0,r.unit_en,r.unit) as unit ,c.order_id,u.googleMap,u.displayName,o.city as address,ifnull(f.nickname,u.name) as customerName ,o.logistic_sequence_No,o.logistic_truck_No,c.bonus_title,c.customer_buying_quantity,r.unit  from ";
