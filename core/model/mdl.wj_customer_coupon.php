...
class mdl_wj_customer_coupon extends mdl_base
{
	...
	function getItemsInOrder($order_id,$business_id){
		...
		$sql =" select c.id,if(length(m.menu_en_name)>0,m.menu_en_name,c.bonus_title) as bonus_title,c.message,c.bonus_type,c.bonus_type_name,c.menu_id,c.new_customer_buying_quantity as customer_buying_quantity,
...
//var_dump($sql);exit;
    ...
    function getOrderItemsWithReturnInfo($orderId) {

        $sql ="select a.*,
       if(r.adjust_quantity is null,a.new_customer_buying_quantity,r.adjust_quantity) as adjust_quantity,
       if(r.adjust_price is null ,a.voucher_deal_amount,r.adjust_price) as adjust_price ,
      if(r.returnType is null ,0,r.returnType) as returnType ,
    ...
    ,m.menu_en_name,m.menu_cn_name, if(length(m.unit_en)>0,m.unit_en,m.unit) as unit  from cc_wj_customer_coupon a 
        ...
        foreach ($list as $key=>$value){
            ...
            $list[$key]['credit'] =number_format($value['new_customer_buying_quantity']*$value['voucher_deal_amount']-$value['adjust_quantity']*$value['adjust_price'],2);
        }
        return $list;
    }
