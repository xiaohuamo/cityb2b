...
class mdl_infoClass extends mdl_base
{
	...
	public function getParentNameStr ($class_id, $split_str = ', ')
	{
		...
		$list	= $this->db->toArray($this->db->select('name', $this->tableName, $where, "id asc"));
	...
	public function getInfoCount ($class_id, $isApproved = -1)
	{
		...
		return $this->db->getCountBySql("select count(*) as cnt from #@_info where".($isApproved >= 0 ? " isApproved='{$isApproved}' and" : "")." classId like '$class_id%'");
	}
	...
	public function getChild ($class_id, $level = 20)
	{
		...
		$re = $this->db->select('id, ordinal, name, url, imageUrl, files, defaultDisplayMode, extend, alias, coupontype', $this->tableName, $where, "Ordinal asc");
	...
	public function getChildForPermission ($class_id, $level = 20)
	{
		//在数据中同级别保存，用level获取真实级别
		static $classArray = array();
		$class_id = $GLOBALS['system']->chkClass($class_id);
	...
	public function getChild4 ($class_id, $level = 20, $show_in_nav = null)
	{
		//在数据中分级别保存
		//static $classArray = array();
		$classArray = array();
		...
		$re = $this->db->select('id, name, alias, imageUrl', $this->tableName, $where, "ordinal asc");
		while ($ro = $this->db->fetch_array($re))
		{
			$childCount	= $this->getChildCount($ro['id']);
			$classArray[$ro['id']] = array(
	...
	public function getChild5( $class_id, $cnt ) {
		...
		return $this->db->toArray( $this->db->select('id, name, alias', $this->tableName, $where, "ordinal asc", $cnt) );
	}
	...
	public function update ($data, $class_id)  //由于文件上传中涉及到信息分类图片上传，所以此处写个固定的更新方法
	{
