...
(function($)
{
	$.fn.droplist = function (opt, callback, visibilityCallback)
	{
		...
		_this.find('option').each(function(i){
			...
			_listObj.find('dl').append('<dt class="item clearfix">'+ ( _multiple && _child == 0 ? '<input type="checkbox"'+ ( $(this).attr('selected') ? ' checked' : '' ) +' />' : '' ) +'<a href="'+ $(this).val() +'">'+ $(this).html() +'</a></dt>');
		});
		if ( _multiple ) {
			var selectedLength = _this.find('option:selected').length;
			if ( selectedLength > 1 ) {
				_ddtObj.attr('href', '#').find('span').html('选择了'+ selectedLength +'个');
			}
